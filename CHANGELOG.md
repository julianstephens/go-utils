...
- helpers: improves atomic file operations robustness and deprecates string pointer helper
  - improves `AtomicFileWrite()` to handle short writes correctly using `io.Copy` internally
    - prevents partial writes on systems where `os.File.Write()` returns `n < len(data)` with `err == nil`
    - ensures complete data durability even on edge-case short write scenarios
  - improves `AtomicFileWrite()` to preserve existing file permissions when overwriting
    - automatically copies permissions from existing target file to new atomic file
    - prevents unintentional permission changes during atomic updates
  - improves both atomic functions with explicit cleanup tracking using `shouldCleanup` flag
    - safer against variable shadowing during future refactoring
    - replaces implicit error variable closure with explicit boolean flag
  - adds comprehensive documentation noting Unix/Linux-only platform support
    - documents Windows incompatibility due to `os.Rename()` semantics
    - provides guidance for platform-specific implementations if needed
  - adds `ExistsWithInfo()` function for checking file existence with metadata
    - returns `(bool, os.FileInfo, error)` for size, mode, and other file attributes
    - useful for permission checks and conditional logic based on file type
  - **Deprecated**: `StringPtr()` function in favor of `generic.Ptr[T]()` for type safety
    - marks function with deprecation notice in documentation and code comments
    - recommends migration to generic version for better type inference
